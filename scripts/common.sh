...
function package() {
    ...
    fpm -a ${PACKAGE_ARCH} -s dir -t deb --name ${PACKAGE_NAME} --version ${VERSION} --chdir ${PACKAGE_DIR} --after-install after-install.sh --before-install before-install.sh -p ${PACKAGE_NAME}_VERSION_ARCH.deb || exit 1
