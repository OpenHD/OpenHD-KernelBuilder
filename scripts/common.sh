...
function package() {
    ...
    fpm -a ${PACKAGE_ARCH} --after-install after-install.sh --before-install before-install.sh -p ${PACKAGE_NAME}_VERSION_ARCH.deb -s dir -t deb --name ${PACKAGE_NAME} --version ${VERSION} --chdir ${PACKAGE_DIR}  || exit 1
