...
if [[ "${PLATFORM}" == "pi" ]]; then
	...
	echo "Copy kernel7"
	pushd ${LINUX_DIR}
	sudo mkdir -p /usr/src/6.1.54-common
	sudo mkdir -p /usr/src/6.1.54-rpi
	sudo mkdir -p ${PACKAGE_DIR}/lib/modules/6.1.54-v7+/
	sudo ln -s /usr/src/6.1.54-common/ ${PACKAGE_DIR}/lib/modules/6.1.54-v7+/source
	sudo ln -s /usr/src/6.1.54-rpi ${PACKAGE_DIR}/lib/modules/6.1.54-v7+/build
	sudo mkdir -p ${PACKAGE_DIR}/usr/src/6.1.54-rpi/include
	sudo mkdir -p ${PACKAGE_DIR}/usr/src/6.1.54-rpi/arch/arm
	echo "$pwd"
	sudo cp -r include/generated ${PACKAGE_DIR}/usr/src/6.1.54-rpi/include
	sudo cp -r include/config ${PACKAGE_DIR}/usr/src/6.1.54-rpi/include
	sudo cp -r arch/arm ${PACKAGE_DIR}/usr/src/6.1.54-rpi/arch/
	sudo cp -r tools ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	sudo cp -r scripts ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	sudo cp -r Module.symvers ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	sudo cp -r Makefile ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	sudo cp -r .kernelvariables ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	sudo cp -r .config ${PACKAGE_DIR}/usr/src/6.1.54-rpi/
	...
	echo "Copy kernel7l"
	pushd ${LINUX_DIR}
	sudo mkdir -p ${PACKAGE_DIR}/lib/modules/6.1.54-v7l+/
	sudo ln -s /usr/src/6.1.54-common/ ${PACKAGE_DIR}/lib/modules/6.1.54-v7l+/source
	sudo ln -s /usr/src/6.1.54-rpi ${PACKAGE_DIR}/lib/modules/6.1.54-v7l+/build
	sudo ls -a
